...
public extension KeyedDecodingContainer {
    ...
    func decodeArray<T: Decodable>(_ key: K) throws -> [T] {
        ...
        do {
            ...
            if Constants.safeArrayDecoding {
                return array
            } else {
                throw error
            }
    ...
    fileprivate func decodeOptional<T>(_ closure: () throws -> T? ) throws -> T? {
        if Constants.safeOptionalDecoding {
            do {
                return try closure()
            } catch {
                return nil
...
extension DateTime {
    ...
    static let dateEncodingFormatter = DateFormatter(formatString: "yyyy-MM-dd'T'HH:mm:ssZZZZZ",
...
public extension Date {
    func encode() -> Any {
        return DateTime.dateEncodingFormatter.string(from: self)
    }
}
...
enum Constants {
    ...
    static var safeOptionalDecoding = false

    /// Whether to remove invalid elements instead of throwing when decoding arrays
    static var safeArrayDecoding = false
}
