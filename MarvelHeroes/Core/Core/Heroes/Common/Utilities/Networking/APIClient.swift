...
public class APIClient {
    ...
    public init(baseURL: String, session: Session = .default, defaultHeaders: [String: String] = [:], behaviours: [RequestBehaviour] = []) {
        ...
        jsonEncoder.dateEncodingStrategy = .formatted(DateTime.dateEncodingFormatter)
    }
    ...
    private func makeNetworkRequest<T>(request: APIRequest<T>, urlRequest: URLRequest, cancellableRequest: CancellableRequest, requestBehaviour: RequestBehaviourGroup, completionQueue: DispatchQueue, complete: @escaping (APIResponse<T>) -> Void) {
        ...
        if request.service.isUpload {
            let networkRequest = session.upload(
...
extension APIRequest {
    /// Custom query string encoder
    var queryParameterEncoder: URLEncoding {
    ...
    public func createURLRequest(baseURL: URL, encoder: RequestEncoder = JSONEncoder()) throws -> URLRequest {
        ...
        if !queryParams.isEmpty {
            urlRequest = try queryParameterEncoder.encode(urlRequest, with: queryParams)
        }
